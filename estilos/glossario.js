// Função que ativa o glossário em um container (padrão: documento inteiro)
function ativarGlossario(container = document) {
  container.querySelectorAll(".termo").forEach(termo => {
    // Evita duplicar eventos nos termos já processados
    ...
    termo.addEventListener("click", (e) => {
      ...
      if (lista) {
        const jaExiste = [...lista.querySelectorAll("li")]
          .some(li => li.dataset.palavra === palavra);
        if (!jaExiste) {
