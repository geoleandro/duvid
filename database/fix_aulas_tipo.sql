-- =============================================================
--  FIX: adiciona coluna 'tipo' que faltou na criação da tabela
--  Execute no phpMyAdmin com banco "duvid" selecionado
-- =============================================================

USE duvid;

-- 1. Adiciona a coluna 'tipo' se não existir
ALTER TABLE aulas
    ADD COLUMN IF NOT EXISTS tipo ENUM('normal','revisao') NOT NULL DEFAULT 'normal'
    AFTER ano_escolar;

-- 2. Marca as aulas de revisão já existentes (106 se existir)
UPDATE aulas SET tipo = 'revisao' WHERE id IN (106,112,119,126,131,134);

-- 3. 2º ANO — 36 aulas (IDs 201–236)
...
-- 5. Aula 106 do 1º ano que faltou
INSERT IGNORE INTO aulas (id, titulo, ano_escolar, tipo, link_texto, link_questoes, imagem) VALUES
